...
otel_create_active_span_promise_domain <- function(
  ...
  promises_env <- rlang::ns_env("promises")

  promises_env$new_promise_domain(
    ...
    wrapSync = base::force,
    "_local_promise_domain_compat" = TRUE
    # wrapEnter = force,
    # wrapExit = force,
    # wrapSync = FALSE
  )
...
local_promise_domain <- function(
  ...
  if (!identical(domain[["_local_promise_domain_compat"]], TRUE)) {
    if (!identical(domain$wrapSync, base::force)) {
      # If the domain's `wrapSync` is not `base::force`, then we assume it is a custom domain
      # a custom domain that has been created by the otel package.
    ...
    stop(
      "local_promise_domain() is only compatible with `otel_create_active_span_promise_domain(active_span)`.",
      call. = FALSE
    )
  }

  promises_env <- rlang::ns_env("promises")
  promises_globals <- promises_env$globals
  oldval <- promises_env$current_promise_domain()
  if (replace) {
    promises_globals$domain <- domain
  } else {
    new_domain <- promises_env$compose_domains(
      promises_globals$domain,
      domain
    )

    promises_globals$domain <- new_domain
  }

  defer(
    {
      promises_globals$domain <- oldval
    },
    envir = local_envir
  )
}
